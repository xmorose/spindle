  ...
  <div v-if="items.length" class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
    <div v-for="it in items" :key="it.id" data-tile class="group">
      <div class="relative">
        ...
        <button
          v-if="shareable"
          class="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-full bg-[oklch(0.12_0.02_50/0.6)] text-white backdrop-blur transition-colors hover:bg-[oklch(0.12_0.02_50/0.8)]"
