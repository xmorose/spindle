...
function pickDay(i: number) {
  const h = data.value?.history?.[i];
  if (h) pickedDay.value = h.day;
}
...
<template>
  <div class="pb-2">
    <div v-if="loading">
      <div class="-mx-4 -mt-7 mb-9 px-4 pb-8 pt-10 sm:-mx-8 sm:px-8">
        <div class="flex flex-col gap-7 sm:flex-row sm:items-end sm:gap-14">
          <Skeleton class="aspect-square w-[190px] flex-none rounded-xl sm:w-[230px]" />
          <div class="flex-1">
            <Skeleton class="h-2.5 w-20" />
            <Skeleton class="mt-3 h-10 w-80 max-w-full" />
            <Skeleton class="mt-3 h-3 w-44 max-w-full" />
            <div class="mt-7 flex flex-wrap gap-x-10 gap-y-6">
              <div>
    ...
    <div v-else class="stagger">
      <header class="relative -mx-4 -mt-7 mb-9 overflow-hidden px-4 pb-8 pt-10 sm:-mx-8 sm:px-8">
        <CoverBackdrop :id="coverId" :tint="0.68" />

        <div class="relative flex flex-col gap-8 sm:flex-row sm:items-end sm:gap-14">
          <component
            :is="queueTracks.length ? 'button' : 'div'"
            class="w-[190px] flex-none sm:w-[230px]"
            :class="queueTracks.length ? 'cursor-pointer' : ''"
            :aria-label="queueTracks.length ? 'Play' : undefined"
            @click="queueTracks.length && player.playQueue(queueTracks, 0)"
          >
            <VinylHero :id="coverId" :name="data.name" class="w-full" />
          </component>

          <div class="min-w-0 flex-1">
            <div class="label-sm label-over">{{ kind }}<span v-if="data.rank > 0"> · rank #{{ data.rank }}</span></div>
            <h1 class="mt-1.5 text-4xl font-black leading-[0.95] tracking-tight sm:text-6xl">{{ kind === 'artist' ? cleanArtist(data.name) : data.name }}</h1>
            <RouterLink v-if="data.artist && kind !== 'artist' && data.artistId" :to="`/artists/${data.artistId}`"
              class="mt-2 inline-block text-[15px] text-muted transition-colors hover:text-text hover:underline">{{ cleanArtist(data.artist) }}</RouterLink>
            <div v-else-if="data.artist && kind !== 'artist'" class="mt-2 text-[15px] text-muted">{{ cleanArtist(data.artist) }}</div>

            <div class="mt-7 flex flex-wrap items-end gap-x-10 gap-y-6">
              <div class="flex-none">
                <div class="tabular text-4xl font-black leading-none tracking-tight" :style="{ color: 'var(--accent)' }">{{ formatNumber(data.plays) }}</div>
                <div class="label-sm label-over mt-2">Plays</div>
              </div>
              <div class="flex-none">
                <div class="tabular text-2xl font-black leading-none tracking-tight">{{ formatDuration(data.seconds) }}</div>
                <div class="label-sm label-over mt-2">Listening time</div>
              </div>
              <div class="flex gap-10 sm:border-l sm:border-[oklch(0.97_0.02_80/0.18)] sm:pl-10">
                <div>
                  ...
                  <div class="label-sm label-over mt-2">First play</div>
                </div>
                <div>
                  ...
                  <div class="label-sm label-over mt-2">Last play</div>
                </div>
              </div>
            </div>

            <div v-if="queueTracks.length" class="mt-7 flex flex-wrap gap-2">
              <button
      ...
      <p v-if="noTimestamps" class="mb-9 -mt-4 text-xs text-faint">
      ...
      <section v-if="historyValues.length" class="mb-9">
        <div class="label mb-3">Plays over time</div>
        <LineArea :values="historyValues" :labels="historyLabels" :height="140" zoomable pickable @pick="pickDay" />
      </section>

      <section v-if="relatedRows.length">
        <div class="label mb-3">{{ kind === 'track' ? 'Related' : 'Tracks' }}</div>
